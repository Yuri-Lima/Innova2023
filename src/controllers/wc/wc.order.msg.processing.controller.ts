 ...
 * @returns {Promise<any>}
 */
const httpOrderProcessingMsg = async (req: Request, res: Response): Promise<any> => {
    try {
        const body = req.body;
        const msg_id = await getLatestMsgId(OrderProcessingMsgModel);
        body.msg_id = msg_id + 1;
        const msgData = await OrderProcessingMsgModel.create(body);

        if (msgData === null) {
            return res.status(400).json({
                message: 'Error creating order Processing Message'
            });
        }

        console.log('httpOrderProcessingMsg', msgData);
 ...
 * @returns {Promise<any>}
 */
const httpOrderProcessingGetMsgbyId = async (req: Request, res: Response): Promise<any> => {
    try {
        if (req.params.msg_id === undefined) {
            console.error('httpOrderProcessingGetMsgbyId - msg_id undefined on the request params');
            return res.status(400).json({
                message: 'Order Created Message ID Not Found'
            });
        }
        const msgData = await OrderProcessingMsgModel.findOne(
        {
        ...
        console.log('httpOrderProcessingGetMsgbyId', msgData);
        return res.status(201).json({
 ...
 * @returns {Promise<any>}
 */
const httpOrderProcessingGetAllMsg = async (_req: Request, res: Response): Promise<any> => {
    try {
        ...
        const msgData = await OrderProcessingMsgModel.find(
        ...
        console.log('httpOrderProcessingGetAllMsg', msgData);
        return res.status(201).json({
 ...
 * @returns {Promise<any>}
 */
const httpOrderProcessingSaveMsg = async (req: Request, res: Response): Promise<any> => {
    try {
        const body = req.body;
        if (body.msg_id === undefined) {
            console.error('httpOrderProcessingSaveMsg ID is undefined', body);
            return res.status(400).json({
                message: 'Must provie ID msg'
            });
        }
        const msgData = await OrderProcessingMsgModel.updateOne(
            {
        ...
        if (msgData === null) {
            console.error('httpOrderProcessingSaveMsg msgData is null', msgData);
            return res.status(400).json({
                message: 'Something went wrong'
            });
        }
        if (msgData.matchedCount === 0) {
            console.error('httpOrderProcessingSaveMsg ID does not match any', msgData);
            return res.status(400).json({
        ...
        console.log('httpOrderProcessingSaveMsg', req.body);
 ...
 * @returns {Promise<any>}
 */
const httpOrderProcessingDeleteMsg = async (req: Request, res: Response): Promise<any> => {
    try {
        if (req.params.msg_id === undefined) {
            console.error('httpOrderProcessingDeleteMsg - msg_id undefined on the request params');
            return res.status(400).json({
                message: 'Must provie ID msg'
            });
        }

        const msgData = await OrderProcessingMsgModel.deleteOne( // deleteOne() returns the number of deleted documents
            {
                msg_id: req.params.msg_id
            }
        );
        if (msgData === null) {
            console.error('httpOrderProcessingDeleteMsg null', msgData);
            return res.status(400).json({
                message: 'Something went wrong'
            });
        }
        if (msgData.deletedCount === 0) {  
            console.error('httpOrderProcessingDeleteMsg deletedCount', msgData); 
            return res.status(400).json({
                message: 'Does not match any ID'
            });
        }
        console.log('httpOrderProcessingDeleteMsg', msgData);
        return res.status(201).json({
 ...
 */
const httpOrderProcessingDeleteAllMsg = async (_req: Request, res: Response ): Promise<any> => {
    try {
        const msgData = await OrderProcessingMsgModel.deleteMany( // deleteMany() returns the number of deleted documents
            {}
        );
        if (msgData === null) {
            console.error('httpOrderProcessingDeleteAllMsg null', msgData);
            return res.status(400).json({
                message: 'Something went wrong'
            });
        }
        if (msgData.deletedCount === 0) {
            console.error('httpOrderProcessingDeleteAllMsg deletedCount', msgData);
            return res.status(400).json({
                message: 'Does not match any ID'
            });
        }
        console.log('httpOrderProcessingDeleteAllMsg', msgData);
        return res.status(201).json({
            message: 'Msg Created Order Deleted',
            msgData
        });
